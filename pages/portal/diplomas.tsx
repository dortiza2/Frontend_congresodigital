...
const DiplomasPage = () => {
  ...
  const loadData = async () => {
    try {
      ...
      const certificates: Certificate[] = certificatesResponse.data.map((cert: any) => ({
        id: cert.id,
        userId: cert.userId,
        activityId: cert.activityId || '',
        activityTitle: cert.activityTitle || 'Actividad',
        activityType: 'taller',
        completedAt: new Date(cert.createdAt),
  ...
  const generateCertificate = async (certificateId: string) => {
    ...
    try {
      ...
      const data = await apiClient.post(`/student/certificates/${certificateId}/generate`) as any;
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {certificates.length === 0 ? (
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-8 text-center">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">No tienes certificados</h3>
            <p className="text-gray-600 mb-4">Completa actividades para obtener certificados.</p>
            <a
                  ...
                  <div className="flex-1">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-gray-600">
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <span>Duración: {certificate.hours} hora{certificate.hours !== 1 ? 's' : ''}</span>
                      </div>
                    ...
                    {!hasConfirmedAttendance(certificate.activityId) ? (
                      <>
                        <div className="flex items-center space-x-2 px-4 py-2 bg-gray-100 text-gray-600 rounded-lg">
                          ...
                          <span className="text-sm">Disponible tras confirmar asistencia</span>
                        </div>
                        <button
                          disabled
                          className="flex items-center justify-center space-x-2 px-4 py-2 bg-gray-300 text-gray-500 rounded-lg cursor-not-allowed"
                        >
                          <Award className="h-4 w-4" />
                          <span>Diploma no disponible</span>
                        </button>
                      </>
                    ) : certificate.isGenerated ? (
                      <>
